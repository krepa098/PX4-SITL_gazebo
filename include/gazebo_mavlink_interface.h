...
typedef const boost::shared_ptr<const physics_msgs::msgs::UniversalGripperCommand> UGCommandPtr;
typedef const boost::shared_ptr<const physics_msgs::msgs::UniversalGripperStatus> UGStatusPtr;
...
static const std::string kDefaultUGCommandTopic = "/command";
static const std::string kDefaultUGStatusTopic = "/status";
...
static inline uint32_t operator|(A lhs, B rhs)
{
    ...
    return static_cast<uint32_t>(static_cast<std::underlying_type<SensorSource>::type>(lhs) |
...
class GazeboMavlinkInterface : public ModelPlugin
{
   ...
   private:
    ...
    void UGStatusCallback(UGStatusPtr& msg);
    ...
    void CreateSensorSubscription(void (GazeboMavlinkInterface::*fp)(const boost::shared_ptr<GazeboMsgT const>&,
                                                                     const int&),
    ...
    transport::PublisherPtr ug_command_pub_;
    ...
    transport::SubscriberPtr ugstatus_sub_;
