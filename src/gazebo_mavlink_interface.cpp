...
struct SensorHelperStorage
{
    ...
    void callback(const boost::shared_ptr<GazeboMsgT const>& msg_ptr) { (ptr->*fp)(msg_ptr, sensor_id); }
};
...
void GazeboMavlinkInterface::CreateSensorSubscription(
    void (GazeboMavlinkInterface::*fp)(const boost::shared_ptr<GazeboMsgT const>&, const int&),
    GazeboMavlinkInterface* ptr,
    const physics::Joint_V& joints,
    const std::regex& model)
{
    ...
    for (physics::Joint_V::const_iterator it = joints.begin(); it != joints.end(); ++it)
    {
        if (std::regex_match((*it)->GetName(), model))
        {
            ...
            auto callback_entry = callback_map.emplace("~/" + model_name + "/link/" + sensor_name,
            ...
            auto subscriberPtr =
                node_handle_->Subscribe("~/" + model_name + "/link/" + sensor_name,
                                        &SensorHelperStorage<GazeboMsgT>::callback, &callback_entry.first->second);

            // Store the SubscriberPtr, sensor ID and sensor orientation
            sensor_map_.insert(std::pair<transport::SubscriberPtr, SensorIdRot_P>(
                subscriberPtr, SensorIdRot_P(sensor_id, sensor_orientation)));
        }
    }
}
...
void GazeboMavlinkInterface::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
    ...
    if (enable_lockstep_)
    {
        ...
        {
            gzerr << "max_step_size of " << max_step_size << " s does not match real_time_update_rate of "
    ...
    opticalFlow_sub_ = node_handle_->Subscribe("~/" + model_->GetName() + opticalFlow_sub_topic_,
                                               &GazeboMavlinkInterface::OpticalFlowCallback, this);
    irlock_sub_ = node_handle_->Subscribe("~/" + model_->GetName() + irlock_sub_topic_,
                                          &GazeboMavlinkInterface::IRLockCallback, this);
    groundtruth_sub_ = node_handle_->Subscribe("~/" + model_->GetName() + groundtruth_sub_topic_,
                                               &GazeboMavlinkInterface::GroundtruthCallback, this);
    vision_sub_ = node_handle_->Subscribe("~/" + model_->GetName() + vision_sub_topic_,
                                          &GazeboMavlinkInterface::VisionCallback, this);
    mag_sub_ = node_handle_->Subscribe("~/" + model_->GetName() + mag_sub_topic_,
                                       &GazeboMavlinkInterface::MagnetometerCallback, this);
    airspeed_sub_ = node_handle_->Subscribe("~/" + model_->GetName() + airspeed_sub_topic_,
                                            &GazeboMavlinkInterface::AirspeedCallback, this);
    baro_sub_ = node_handle_->Subscribe("~/" + model_->GetName() + baro_sub_topic_,
    ...
    // Publish UG commands
    //if (_sdf->HasElement("ug") && _sdf->GetElement("ug")->Get<bool>())
    {
      ug_command_pub_ =
        node_handle_->Advertise<physics_msgs::msgs::UniversalGripperCommand>("~/" + model_->GetName() + kDefaultUGCommandTopic, 1);

      ugstatus_sub_ =
        node_handle_->Subscribe("~/" + model_->GetName() + kDefaultUGStatusTopic, &GazeboMavlinkInterface::UGStatusCallback, this);
    }
...
void GazeboMavlinkInterface::SendSensorMessages()
{
    ignition::math::Quaterniond q_gr =
        ignition::math::Quaterniond(last_imu_message_.orientation().w(), last_imu_message_.orientation().x(),
                                    last_imu_message_.orientation().y(), last_imu_message_.orientation().z());
    ...
    ignition::math::Vector3d accel_b = q_FLU_to_FRD.RotateVector(ignition::math::Vector3d(
        last_imu_message_.linear_acceleration().x(), last_imu_message_.linear_acceleration().y(),
        last_imu_message_.linear_acceleration().z()));

    ignition::math::Vector3d gyro_b = q_FLU_to_FRD.RotateVector(
        ignition::math::Vector3d(last_imu_message_.angular_velocity().x(), last_imu_message_.angular_velocity().y(),
...
void GazeboMavlinkInterface::SendGroundTruth()
{
    ...
    ignition::math::Vector3d omega_nb_b =
        q_FLU_to_FRD.RotateVector(ignitionFromGazeboMath(model_->GetRelativeAngularVel()));
#endif
    ...
    ignition::math::Vector3d accel_true_b =
        q_FLU_to_FRD.RotateVector(ignitionFromGazeboMath(model_->GetRelativeLinearAccel()));
#endif
...
void GazeboMavlinkInterface::LidarCallback(LidarPtr& lidar_message, const int& id)
{
    ...
    ignition::math::Quaterniond q_ls =
        ignition::math::Quaterniond(lidar_message->orientation().w(), lidar_message->orientation().x(),
                                    lidar_message->orientation().y(), lidar_message->orientation().z());
    ...
    const ignition::math::Vector3d u_Xb = kForwardRotation;  // This is unit vector of X-axis `base_link`
    const ignition::math::Vector3d u_Xs =
        q_bs.RotateVectorReverse(u_Xb);  // This is unit vector of X-axis sensor in `base_link` frame
...
void GazeboMavlinkInterface::SonarCallback(SonarPtr& sonar_message, const int& id)
{
    ...
    const ignition::math::Vector3d u_Xb = kForwardRotation;  // This is unit vector of X-axis `base_link`
    const ignition::math::Vector3d u_Xs =
        q_ls.RotateVectorReverse(u_Xb);  // This is unit vector of X-axis sensor in `base_link` frame
...
void GazeboMavlinkInterface::VisionCallback(OdomPtr& odom_message)
{
    ...
    // the ENU earth/local
    ignition::math::Quaterniond q_gr =
        ignition::math::Quaterniond(odom_message->orientation().w(), odom_message->orientation().x(),
                                    odom_message->orientation().y(), odom_message->orientation().z());
    ...
    ignition::math::Vector3d linear_velocity = q_FLU_to_FRD.RotateVector(q_gr.Inverse().RotateVector(
        ignition::math::Vector3d(odom_message->linear_velocity().x(), odom_message->linear_velocity().y(),
                                 odom_message->linear_velocity().z())));

    // transform angular velocity from body FLU to body FRD frame
    ignition::math::Vector3d angular_velocity = q_FLU_to_FRD.RotateVector(
        ignition::math::Vector3d(odom_message->angular_velocity().x(), odom_message->angular_velocity().y(),
...
void GazeboMavlinkInterface::MagnetometerCallback(MagnetometerPtr& mag_msg)
{
    ...
    mag_n_ = ignition::math::Vector3d(mag_msg->magnetic_field().x(), mag_msg->magnetic_field().y(),
                                      mag_msg->magnetic_field().z());

    mag_updated_ = true;
}

void GazeboMavlinkInterface::UGStatusCallback(UGStatusPtr& ug_status_msg)
{
    ...
    mavlink_message_t msg;
    mavlink_msg_universal_gripper_status_gazebo_encode_chan(1, 200, MAVLINK_COMM_0, &msg, &ug_status_gz);
    send_mavlink_message(&msg);
}
...
void GazeboMavlinkInterface::pollForMAVLinkMessages()
{
    ...
    } while (!close_conn_ && received_first_actuator_ && !received_actuator && enable_lockstep_ && IsRunning() &&
             !gotSigInt_);
}
...
void GazeboMavlinkInterface::handle_message(mavlink_message_t* msg, bool& received_actuator)
{
    switch (msg->msgid)
    {
        ...
        break;
        case MAVLINK_MSG_ID_UNIVERSAL_GRIPPER_COMMAND:
        {
            mavlink_universal_gripper_command_t ug_command;
            mavlink_msg_universal_gripper_command_decode(msg, &ug_command);

            physics_msgs::msgs::UniversalGripperCommand ug_command_msg;
            ug_command_msg.set_command(uint32_t(ug_command.command));
            ug_command_pub_->Publish(ug_command_msg);
        }
        break;
    }
}
...
{
    serial_dev.async_read_some(boost::asio::buffer(rx_buf), boost::bind(&GazeboMavlinkInterface::parse_buffer, this,
